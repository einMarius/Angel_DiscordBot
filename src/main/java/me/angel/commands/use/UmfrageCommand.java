...
public class UmfrageCommand implements ServerCommand {
    ...
    public void performCommand(Member member, TextChannel textChannel, Message message) {
        ...
        if(member.hasPermission(Permission.MANAGE_PERMISSIONS)) {
            if(textChannel.getIdLong() == plugin.ABSTIMMUNGEN || textChannel.getIdLong() == 851131058178097172L){
                if(args.length >= 2) {
                    ...
                    message.delete().submit()
                            ...
                            .whenCompleteAsync((s, error) -> {
                                textChannel.sendMessage("||<@&"+plugin.UMFRAGE+">|| \n").embed(abstimmung.build()).queue((embedMessage) -> {
